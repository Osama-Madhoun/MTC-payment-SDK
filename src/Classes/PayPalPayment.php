...
class PayPalPayment extends BaseController implements PaymentInterface
{
    ...
    {
        $this->paypal_client_id = config('MTC-payments.PAYPAL_CLIENT_ID');
        $this->paypal_secret = config('MTC-payments.PAYPAL_SECRET');
        $this->verify_route_name = config('MTC-payments.VERIFY_ROUTE_NAME');
        $this->paypal_mode = config('MTC-payments.PAYPAL_MODE');
        $this->currency = config('MTC-payments.PAYPAL_CURRENCY');
    }
